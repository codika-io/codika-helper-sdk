...
```bash
codika notes upsert h8iCqSgTjSsKySyufq36 --type test-fail --profile cli-test-owner-full --json
```
...
```bash
codika notes upsert h8iCqSgTjSsKySyufq36 --type test-fail --summary "test" --profile cli-test-owner-full --json
```
...
**Why**: Project note operations use the `deploy:use-case` scope.
...
After running tests, remove test documents from the project. (Project notes don't have a delete endpoint yet, so this is manual Firestore cleanup if needed.)
