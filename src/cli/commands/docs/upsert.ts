 ...
 * Docs Upsert Command
 *
 * Create or update an agent document for a project.
 *
 * Usage:
 *   codika docs upsert <projectId> --type brief --title "Project Brief" --content "..." --summary "Initial"
 *   codika docs upsert <projectId> --type brief --file ./docs/brief.md --summary "Updated brief"
 */
...
export const upsertCommand = new Command('upsert')
  .description('Create or update an agent document')
...
async function runUpsert(projectId: string, options: UpsertOptions): Promise<void> {
  const apiUrl = resolveEndpointUrl('upsertAgentDocument', options.apiUrl, options.profile);
  ...
  if (!content) {
    exitWithError('Either --content or --file is required');
  }
